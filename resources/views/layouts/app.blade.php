    ...
    <script>
        ...
        window.themeManager = function() {
            return {
                ...
                sidebarOpen: localStorage.getItem('sidebar_state') !== 'collapsed',
                ...
                init() {
                    ...
                    // Watch for sidebar state changes and persist
                    this.$watch('sidebarOpen', (val) => {
                        localStorage.setItem('sidebar_state', val ? 'open' : 'collapsed');
                    });
                }
            }
        }
            ...
            <header class="lg:hidden sticky top-0 z-30 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 px-4 py-3">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center gap-1">
                        <!-- Dark Mode Toggle -->
                        <button @click="darkMode = !darkMode" class="p-2 rounded-lg text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700 transition">
                            <svg x-show="!darkMode" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z"/>
                            </svg>
                            <svg x-show="darkMode" x-cloak class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <a href="{{ route('profile.edit') }}" 
                           class="p-2 rounded-lg transition {{ request()->routeIs('profile.*') ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-700' }}">
                            <div class="w-7 h-7 bg-gradient-to-br from-emerald-400 to-teal-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                                {{ substr(Auth::user()->name ?? 'U', 0, 1) }}
                            </div>
                        </a>
                    </div>
            ...
            <div class="flex items-center justify-around h-16">
                @php
                    $mobileNav = [
                        ...
                        ['route' => 'ai-recommendation', 'icon' => 'chart-pie', 'label' => 'Analisis', 'match' => 'ai-recommendation'],
                    ];
                @endphp
